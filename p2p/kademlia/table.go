...
type transport interface {
	ping(p2p.NetAddress) error
	findnode(toid p2p.NetAddress, target p2p.NodeID) ([]*Node, error)
	close()
}
...
func newTable(t transport, ourID p2p.NodeInfo, nodeDBPath string, bootnodes []*Node, logger log.Logger) (*Table, error) {
	...
	tab := &Table{
		net:        t,
		db:         db,
		self:       NewNode(ourID.ID, ourID.NetAddress().IP),
...
func (tab *Table) setFallbackNodes(nodes []*Node) error {
	for _, n := range nodes {
		if err := n.validateComplete(); err != nil {
			return fmt.Errorf("bad bootstrap/fallback node %q (%v)", hex.EncodeToString(n.ID.ID[:]), err)
		}
	}
...
func (tab *Table) lookup(targetID p2p.NodeID, refreshIfEmpty bool) []*Node {
	var (
		asked          = make(map[p2p.NodeID]bool)
	...
	for {
		...
		result = tab.closest(targetID.ID, bucketSize)
		tab.mutex.Unlock()
...
func (tab *Table) findnode(n *Node, targetID p2p.NodeID, reply chan<- []*Node) {
	fails := tab.db.findFails(n.ID)
	r, err := tab.net.findnode(*p2p.NewNetAddress(n.ID, n.addr()), targetID)
	if err != nil || len(r) == 0 {
		fails++
...
func (tab *Table) doRefresh(done chan struct{}) {
	...
	// Run self lookup to kademlia new neighbor nodes.
	...
	for i := 0; i < 3; i++ {
		var target p2p.NodeID
		crand.Read(target.ID[:])
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	err := tab.net.ping(*p2p.NewNetAddress(last.ID, last.addr()))
...
func (tab *Table) delete(node *Node) {
	...
	tab.deleteInBucket(tab.bucket(node.ID.ID), node)
}
...
type nodesByDistance struct {
	entries []*Node
	target  [NodeIDBits / 8]byte
}

// push adds the given node to the list, keeping the total size below maxElems.
func (h *nodesByDistance) push(n *Node, maxElems int) {
	ix := sort.Search(len(h.entries), func(i int) bool {
		return distcmp(h.target, h.entries[i].ID.ID, n.ID.ID) > 0
	})

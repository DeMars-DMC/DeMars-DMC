...
The `msgPacket` is serialized using [go-wire](https://github.com/demars-dmc/go-wire) and prefixed with 0x3.
...
using the `demars/wire` submodule's `WriteBinary()` reflection routine.

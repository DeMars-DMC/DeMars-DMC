# DéMars Peer Discovery

A DéMars P2P network has different kinds of nodes with different requirements for connectivity to one another.
This document describes what kind of nodes DéMars should enable and how they should work.
...
See [the peer-exchange docs](https://github.com/demars-dmc/demars-dmc/docs/spec/reactors/pex/pex.md)for details on peer quality.

## New Full Node

A new node needs a few things to connect to the network:
- a list of seeds, which can be provided to DéMars via config file or flags,
...
The values `H` and `HASH` must be received and corroborated by means external to DéMars, and specific to the user - ie. via the user's trusted social consensus.
...
dials those peers, and runs the DéMars protocols with those it successfully connects to.

When the peer catches up to height H, it ensures the block hash matches HASH.
If not, DéMars will exit, and the user must try again - either they are connected

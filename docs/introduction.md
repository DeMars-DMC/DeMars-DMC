...
<!--
Demars-DMC is software for securely and consistently replicating an
application on many machines. By securely, we mean that Demars-DMC works
...
Demars-DMC consists of two chief technical components: a blockchain
consensus engine and a generic application interface. The consensus
engine, called Demars-DMC Core, ensures that the same transactions are
...
language), developers can use Demars-DMC for BFT state machine
replication of applications written in whatever programming language and
development environment is right for them.

Demars-DMC is designed to be easy-to-use, simple-to-understand, highly
performant, and useful for a wide variety of distributed applications.

## Demars-DMC vs. X

Demars-DMC is broadly similar to two classes of software. The first
...
Demars-DMC is in essence similar software, but with two key differences:
...
The layout of this Demars-DMC website content is also ripped directly
and without shame from [consul.io](https://www.consul.io/) and the other
[Hashicorp sites](https://www.hashicorp.com/#tools).

### Bitcoin, Ethereum, etc.

Demars-DMC emerged in the tradition of cryptocurrencies like Bitcoin,
...
Demars-DMC had a simple currency built in, and to participate in
...
Demars-DMC a Proof-of-Stake algorithm.

Since then, Demars-DMC has evolved to be a general purpose blockchain
...
application using Demars-DMC consensus. Indeed, [we did that with
Ethereum](https://github.com/Demars-DMC/ethermint). And we plan to do
the same for Bitcoin, ZCash, and various other deterministic
applications as well.

Another example of a cryptocurrency application built on Demars-DMC is
[the Cosmos network](http://cosmos.network).

### Other Blockchain Projects

[Fabric](https://github.com/hyperledger/fabric) takes a similar approach
to Demars-DMC, but is more opinionated about how the state is managed,
...
Demars-DMC, though extending Demars-DMC to handle non-determinism
...
contracts, and an alternative blockchain API. It uses Demars-DMC as its
consensus engine, and provides a particular application state.

## ABCI Overview

The [Application BlockChain Interface
(ABCI)](https://github.com/Demars-DMC/Demars-DMC/tree/develop/abci)
...
and its primary implementation, the Demars-DMC Socket Protocol (TSP, or
Teaspoon).

### Intro to ABCI

[Demars-DMC Core](https://github.com/Demars-DMC/Demars-DMC) (the
...
one wanted to create a Bitcoin-like system on top of ABCI, Demars-DMC
...
Demars-DMC is able to decompose the blockchain design by offering a very
...
Types](https://github.com/Demars-DMC/Demars-DMC/blob/develop/abci/README.md#message-types).
...
validating transactions. Demars-DMC Core's mempool first checks the
...
Demars-DMC Core creates three ABCI connections to the application; one
...
reached among the Demars-DMC Core replica nodes.
...
Demars-DMC is an easy-to-understand, mostly asynchronous, BFT consensus
...
current proposer may be offline, or the network may be slow. Demars-DMC
...
timeout is what makes Demars-DMC a weakly synchronous protocol, rather
...
Demars-DMC is that it uses the same mechanism to commit a block as it
does to skip to the next round.

Assuming less than one-third of the validators are Byzantine, Demars-DMC
...
Since Demars-DMC can replicate arbitrary applications, it is possible to
...
The following diagram is Demars-DMC in a (technical) nutshell. [See here
for high resolution
